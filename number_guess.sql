...
COPY public.games (game_id, user_id, number_of_guesses) FROM stdin;
\.
...
COPY public.guesses (game_id, guess) FROM stdin;
\.
...
COPY public.users (user_id, name, number_of_games, best_game) FROM stdin;
\.
...
ALTER TABLE ONLY public.guesses
    ADD CONSTRAINT guesses_pkey PRIMARY KEY (game_id, guess);
